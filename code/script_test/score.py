...
gt_root = '/home/user/Database/Database/ADE2017/annotations_sceneparsing/validation/'
pre_root = './predict/validation/'
val_pth = './validation.txt'
n_class = 151
...
def compute_hist(val_list):
    ...
    for idx in val_list:
        #print idx
        label = cv2.imread(gt_root + idx.split('.')[0] + '.png', 0)
        print (gt_root + idx.split('.')[0] + '.png')
        gt = label.flatten()
        tmp = cv2.imread(pre_root + idx.split('.')[0] + '.png', 0)
